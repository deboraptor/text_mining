...
function PerformSearch()
{
	...
	if ( searchText.length < 3 && searchText.length > 0 )
	{
		...
		alert( 'Le texte de recherche doit faire au moins 3 caractères.' );
...
function ValidateVoteSuccess( transport )
{
	...
	{
		ShowAlertDialog( 'Erreur', 'Une erreur est survenue lors du traitement de votre requête : unknown' );
	}
	...
	{
		ShowAlertDialog( 'Erreur', 'Votre compte ne dispose pas des privilèges suffisants pour effectuer cette action. Veuillez vous assurer que vous possédez le jeu et que votre compte est en règle.' );
	}
	else if ( transport.responseJSON.success == 24 )
	{
		ShowAlertDialog( 'Erreur', 'Votre compte ne répond pas aux exigences pour utiliser cette fonctionnalité. <a class="whiteLink" href="https://help.steampowered.com/fr/wizard/HelpWithLimitedAccount" target="_blank" rel="">Visitez le site du Support Steam</a> pour plus d\'informations.' );
	}
	else if ( transport.responseJSON.success == 16 )
	{
		ShowAlertDialog( 'Erreur', 'Un problème a été rencontré lors de l\'envoi de votre requête à nos serveurs. Veuillez réessayer.' );
	}
	else if ( transport.responseJSON.success == 17 )
	{
		ShowAlertDialog( 'Erreur', 'Vous ne pouvez pas évaluer ce contenu car votre compte a reçu un bannissement en jeu ou par VAC.' );
	}
	else if ( transport.responseJSON.success == 20 )
	{
		ShowAlertDialog( 'Erreur', 'Le vote pour cet objet est terminé.' );
	}
	else if ( transport.responseJSON.success != 1 )
	{
		ShowAlertDialog( 'Erreur', 'Une erreur est survenue lors du traitement de votre requête : ' + transport.responseJSON.success );
	}
